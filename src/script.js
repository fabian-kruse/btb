...
class Game {
		...
		while (!this.game_over) {
			...
			while (!this.current_round.isOver) {
				...
				if (this.player_is_major) {
					...
					cTurn = this.model.getAction(
						this.current_round.turn,
						this.current_round.sample
					);
	...
	startRound() {
		...
		//set round number
		document.getElementById('round_number').textContent = (
			this.score[0] +
			this.score[1] +
			1
		).toString();
	...
	playTurn(player, turn) {
		...
		if (turn.toString().includes('guess:')) {
			...
			if (this.current_round.isCorrectConcept(arr)) {
				...
				if (player == 'P') {
					...
					document.getElementsByClassName('c_text')[0].textContent =
						guessReaction[getRandomInt(0, guessReaction.length)] +
						game.current_round.getNameOfGuess(
							arr.filter((el) => {
								return el != '';
							})
						) +
		...
		if (player == 'C')
			console.log(
				'bestGuess:',
				this.current_round.names[this.model.bestGuess()],
				'certainty',
				this.model.posterior[this.model.bestGuess()]
			);
	}
}
...
function setup() {
	...
	input.addEventListener('input', function () {
		input_handler(ctx, game.current_round.sample);
	});
}
...
function updateTableValues() {
	if (game == null) {
		...
		document.getElementsByClassName('player_score')[0].textContent =
			game.score.slice()[0];
		document.getElementsByClassName('computer_score')[0].textContent =
			game.score.slice()[1];
		document.getElementsByClassName('player_score')[2].textContent =
			game.score.slice()[0];
		document.getElementsByClassName('computer_score')[2].textContent =
			game.score.slice()[1];
	}
	document.getElementsByClassName('player_score')[1].textContent =
		totalScore.slice()[0];
	document.getElementsByClassName('computer_score')[1].textContent =
		totalScore.slice()[1];
	document.getElementsByClassName('player_score')[3].textContent =
		totalScore.slice()[0];
	document.getElementsByClassName('computer_score')[3].textContent =
		totalScore.slice()[1];
}
...
function input_handler(ctx, sample) {
	...
	for (let i = 0; i < arr.length; i++) {
		...
		if (!arr[i].includes('z')) {
			...
			//z**4
			if (arr[i].includes('**')) {
				if (arr[i].indexOf('**') > arr[i].indexOf('z')) {
					// x3*z**exponent
					exponent =
						exponent + eval(arr[i].substring(arr[i].indexOf('**') + 2));
					if (arr[i].substring(0, arr[i].indexOf('z')).length != 0) {
						if (arr[i].substring(0, arr[i].indexOf('z')).slice(-1) == '*') {
							//x3*z**exponent
					...
					//x2*base**z
					if (arr[i].substring(0, arr[i].indexOf('**')).includes('*')) {
						//x2*base**z
						base = eval(
							arr[i]
								.substring(0, arr[i].indexOf('**'))
								.substring(
									arr[i].substring(arr[i].indexOf('**')).lastIndexOf('*') + 1
								)
						);
						x2 = eval(
							arr[i]
								...
								.substring(
									0,
									arr[i].substring(0, arr[i].indexOf('**')).lastIndexOf('*')
								)
						);
					} else {
						//base**z
...
function buildConcept(x0, x1, x2, base, x3, exponent) {
	...
	return concept.sort(function (a, b) {
		return a - b;
	});
}
		...
		for (let row = 0; row <= 4; row++) {
			for (let col = 10; col <= 20; col = col + 10) {
				...
				if (row * 20 + col == 100) {
					ctx.fillText(
						row * 20 + col,
						col * grid_size - grid_size / 4,
						y_offset + rowsize * row + rowsize / 2.5
					);
				} else {
					ctx.fillText(
						row * 20 + col,
						col * grid_size - grid_size / 8,
						y_offset + rowsize * row + rowsize / 2.5
					);
				}
			}
		...
		for (let row = 0; row <= 4; row++) {
			for (let col = 1; col <= 20; col++) {
				if (guess.includes(row * 20 + col)) {
					...
					ctx.beginPath();
					ctx.arc(
						col * grid_size + radius / 2,
						y_offset + rowsize * row + radius / 2,
						radius / 2,
						0,
						2 * Math.PI,
						false
		...
		for (let row = 0; row <= 4; row++) {
			for (let col = 1; col <= 20; col++) {
				if (sample.includes(row * 20 + col)) {
					...
					ctx.arc(
						col * grid_size + radius / 2,
						y_offset + rowsize * row - grid_size / 2 + radius / 2,
						radius / 2,
						0,
						2 * Math.PI,
						false
