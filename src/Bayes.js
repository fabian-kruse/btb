class Bayes {
    ...
    updatePosterior(sample) {
        let evidence = 0;
        for (let i = 0; i < this.collection_size; i++) {
            evidence = evidence + this.prior[i]*this.setIndicatorFunction(sample, this.collection[i]) * this.likelihoodFunction(sample, this.collection[i]);
        }
        for (let i = 0; i < this.collection_size; i++) {

            this.posterior[i] = (this.prior[i] * this.setIndicatorFunction(sample,this.collection[i])  *this.likelihoodFunction(sample, this.collection[i])) /evidence;
        }
    }
